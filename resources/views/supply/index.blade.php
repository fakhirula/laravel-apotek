<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('supply.create') }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">Create</a>
    </x-slot>
                        ...
                        <table id="example" class="display" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $supply->medicine->name ?: '-' }}</td>
                                    <td>{{ $supply->quantity ?: '-' }}</td>
                                    <td>{{ $supply->price ?: '-' }}</td>
                                    <td>{{ $supply->tax ?: '-' }}</td>
                                    <td>{{ $supply->total_payment ?: '-' }}</td>
                                    <td>
