                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('transaction.store') }}" method="POST">
                        ...
                        <div class="px-4 sm:px-0">
                            ...
                            <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Transaction details and application.</p>
                        </div>
                        <div class="mt-6 border-t border-gray-100">
                            <dl class="divide-y divide-gray-100">
                                    ...
                                    <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                        <select id="id_medicine" name="id_medicine" autocomplete="id_medicine" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6">
                                            ...
                                            @foreach($medicines as $medicine)
                                                <option value="{{ $medicine->id }}">{{ $medicine->name }} -- {{ ($medicine->price + ($medicine->price * 0.1)) }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                    ...
                                    <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                        <input required value="{{ old('quantity') }}" type="number" min="0" max="{{ $medicine->quantity }}" name="quantity" id="quantity" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6">
